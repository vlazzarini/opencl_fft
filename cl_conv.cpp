...
Clconv::Clconv(cl_device_id device_id, int cvs, int pts,
    ...
    : N(pts << 1), bins(pts), bsize((cvs / pts) * bins), nparts(cvs / pts),
      wp(0), w{NULL, NULL}, w2{NULL, NULL}, b(NULL), fftin(NULL), fftout(NULL),
      buff(NULL), coefs(NULL), in(NULL), context(NULL), commands(NULL),
      program(NULL), fft_kernel(NULL), reorder_kernel(NULL), r2c_kernel(NULL),
      c2r_kernel(NULL), convol_kernel(NULL), olap_kernel(NULL), wgs(0), rwgs(0),
      crwgs(0), rcwgs(0), cvwgs(0), olwgs(0),
  ...
  program = clCreateProgramWithSource(context, 1, (const char **)&fftcode, NULL,
                                      &cl_err);
  if (!program) {
...
int Cldconv::convolution(float *out, float *in) {
  ...
  if (wp > irsize) {
    ...
    clEnqueueWriteBuffer(commands, del, CL_TRUE, wp * sizeof(cl_float),
                                  bytes, in, 0, NULL, NULL);
...
int Cldconv::convolution(float *out, float *in1, float *in2) {
  ...
  if (wp > irsize) {
    int front = wp - irsize;
    bytes = (vsize - front) * sizeof(cl_float);
    clEnqueueWriteBuffer(commands,coefs, CL_TRUE, wp * sizeof(cl_float),
                                  bytes, in2, 0, NULL, NULL);
    bytes = front * sizeof(cl_float);
    clEnqueueWriteBuffer(commands,coefs, CL_TRUE, 0, bytes, &in2[vsize - front],
                         0, NULL, NULL);
  } else
